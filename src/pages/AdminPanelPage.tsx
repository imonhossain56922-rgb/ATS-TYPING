...
export const AdminPanelPage: React.FC = () => {
  ...
  const { content, updateContent, updateImage, resetToDefaults, isAdminLoggedIn, adminRole, adminUsername, logout } = useSiteContent();

  const isAdOnly = adminRole === 'ad_only';

  // Local draft state for editing texts and sections
  const [draft, setDraft] = useState<SiteContent>(content);
  const [activeTab, setActiveTab] = useState<'images' | 'hero' | 'outlets' | 'contact' | 'payment' | 'advertisement'>(() => {
    return isAdOnly ? 'advertisement' : 'images';
  });
  ...
  // Lock to advertisement tab for ad-only user
  useEffect(() => {
    if (isAdOnly && activeTab !== 'advertisement') {
        ...
        <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-amber-400 text-slate-950 flex items-center justify-center font-black shadow-lg shadow-amber-400/20">
              {isAdOnly ? <Megaphone className="w-5 h-5 text-slate-950" /> : <ShieldCheck className="w-6 h-6" />}
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-base sm:text-lg font-bold text-white font-display tracking-tight">
                  {isAdOnly ? 'UAE Typing — Advertisement Panel' : 'UAE Typing — Admin Panel'}
                </h1>
                <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                  ...
                  {isAdOnly ? 'New Advertisement Only' : 'Live Editing'}
                </span>
              </div>
              <p className="text-[11px] text-slate-400">
                {isAdOnly ? (
                  <>Logged in as <strong className="text-amber-300 font-semibold">{adminUsername || 'admin'}</strong> (Ad Manager • Only New Advertisement Option)</>
                ) : (
                  <>Logged in as <strong className="text-amber-300 font-semibold">{adminUsername || 'Admin'}</strong> (Full Editing Permissions)</>
                )}
              </p>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        ...
        <div className="mb-8 p-4 sm:p-5 bg-gradient-to-r from-[#0C1527] to-[#0A101D] border border-amber-500/20 rounded-2xl flex items-start gap-4">
          <div className="p-2.5 rounded-xl bg-amber-400/10 text-amber-400 border border-amber-400/20 flex-shrink-0">
            {isAdOnly ? <Megaphone className="w-5 h-5" /> : <Info className="w-5 h-5" />}
          </div>
          <div className="text-xs sm:text-sm space-y-1">
            <h3 className="font-bold text-white">
              {isAdOnly ? 'New Advertisement Management Activated' : 'Direct CMS & Dimension Inspector Activated'}
            </h3>
            <p className="text-slate-400 leading-relaxed">
              {isAdOnly ? (
                <>
                  Logged in as <strong className="text-amber-300">admin</strong>. You have exclusive access to upload, configure duration, preview, and activate the 720×1280 promotional advertisement popup for the website.
                </>
              ) : (
                <>
        ...
        <div className="flex flex-wrap items-center gap-2 mb-8 border-b border-slate-800 pb-3">
          {/* If Ad-Only user, ONLY show the New Advertisement Option */}
          {!isAdOnly && (
            <>
              <button
          ...
          <button
            ...
            className={`px-4 py-2.5 rounded-xl text-xs sm:text-sm font-bold flex items-center gap-2 transition-all cursor-pointer ${
              activeTab === 'advertisement' || isAdOnly
          ...
          >
            <Megaphone className={`w-4 h-4 ${(activeTab === 'advertisement' || isAdOnly) ? 'text-slate-950' : 'text-amber-400 animate-bounce'}`} />
          ...
          {isAdOnly && (
            <span className="text-xs text-amber-300/80 bg-amber-400/10 border border-amber-400/20 px-3 py-1 rounded-lg">
              Authorized Section: Only New Advertisement Option Active
            </span>
          )}
        </div>
